...
### 🚀 效能優化與上游同步（PostSync v1.5.0）
- **圖片二進位共用快取（`blobCache`）**：
  - 在背景服務中實作圖片記憶體二進位快取（`blobCache`）。
  - 在 Google Drive 轉存等需要下載圖片位元組的多層邏輯中，同一張圖片保證**只向 Facebook/Instagram 圖床下載一次**，大幅節省使用者網路頻寬並避免觸發社群伺服器的下載速率限制（Rate Limit）。
- **原始碼不可見字元靜態掃描工具（`tools/scan_invisible.py`）**：
  - 新增開發者自動化檢查工具，可遞迴掃描所有 JavaScript 檔案，嚴格排查是否存在未跳脫的實體不可見字元（如零寬空格、BiDi 控制碼、ASCII 控制字元）。
  - 全面落實十六進位跳脫規範（`\x00-\x08`、`\u00AD`、`\u200B` 等），永久杜絕作業系統與 Git / grep 將原始碼誤判為二進位檔案的問題。
- **上游功能對齊**：
  - 完整核對並確認與上游 PostSync v1.5.0 最新功能對齊。
...
- **純文字層級過濾 Facebook 導航字與單字字母混淆**：
  ...
  - 排除獨立成行的單一英文字母、數字與符號（如 Facebook 防爬蟲切碎隱形字 `n`、`r`、`o`、`t`、`e`、時間數字與冒號等）；
  - 清洗不可見的雙向文字控制字元（BiDi controls `\u202A-\u202E` 等）；
  - 正文開頭自動去重作者姓名；
